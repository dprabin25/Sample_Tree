...
Pipeline, one call to `python SampleBioShift.py FolderName`:
  ...
  2. build_observed_shifts() (below -- inlined here, no longer a separate
     ...
     then combines across representations (Cell/Cytokines/Microbes) the
     same way the original package's ObservedShifts.py did, and writes the
     result into BioShift_Req/Observed_shifts/.
...
REP_JOB_DIRS = {
    "Cell":      ("Cell", "None", "Bray"),
    "Cytokines": ("Pro1log10", "MinMax", "Bray"),
    "Microbes":  ("Micro", "TSS", "Bray"),
}

def _build_rep_tags(rep_names):
    """Short, unambiguous tags for combined-file names, derived from
    REP_JOB_DIRS itself instead of a hand-maintained dict -- adding or
    renaming a representation there just works. NOT rep[0].upper():
    "Cell" and "Cytokines" both start with "C", which silently collided
    into the same tag ("C201" could mean either representation). First
    three letters is usually enough (Cell->Cel, Cytokines->Cyt,
    Microbes->Mic); if two reps still collide on that, fall back to
    lengthening just the colliding ones until they're distinct.
    """
...
REP_TAG = _build_rep_tags(REP_JOB_DIRS.keys())
...
def build_observed_shifts(base_dir, bioshift_dir):
    ...
    for rep, (file_base, norm, dist) in REP_JOB_DIRS.items():
...
def _run_pipeline_steps(base_dir, bioshift_dir, methods_path_args, log_fh):
        ...
        log_fh.write("\n" + "=" * 80 + "\n[RUN] build_observed_shifts\n" + "=" * 80 + "\n")
        build_observed_shifts(base_dir, bioshift_dir)
