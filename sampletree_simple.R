...
resolve_tree_path <- function(tree_raw){
  ...
  if (!nchar(tree_raw) || tree_raw %in% c("NA","Na","na","0")) return(NA_character_)
...
resolve_existing_csv <- function(path_raw){
  p <- resolve_path(path_raw)
  if (file.exists(p)) return(p)
  if (!grepl("\\.[A-Za-z0-9]+$", basename(path_raw))) {
    p2 <- resolve_path(paste0(path_raw, ".csv"))
    if (file.exists(p2)) return(p2)
  }
  NA_character_
}

align_tree_matrix <- function(mat, ref_tree, label = "matrix"){
  taxa_mat  <- colnames(mat)
  taxa_tree <- ref_tree$tip.label
  
  common <- intersect(taxa_mat, taxa_tree)
  if (length(common) == 0) {
    stop("No overlapping taxa between ", label, " columns and tree tip labels.")
  }
  
  dropped_mat  <- setdiff(taxa_mat, common)
  dropped_tree <- setdiff(taxa_tree, common)
  
  if (length(dropped_mat) > 0) {
    message("  Dropping ", length(dropped_mat), " ", label, " columns not present in tree.")
  }
  if (length(dropped_tree) > 0) {
    message("  Pruning ", length(dropped_tree), " tree tips not present in ", label, ".")
  }
...
req_cols <- c("Profile_Tree","Method_Tree","Boot_Tree",
              "BackBacPhylo_Tree","Profile_Diff","Profile_Library")
if (!all(req_cols %in% names(methods_df))) {
  stop("methods.txt must have columns: ",
...
plot_highlighted_tree <- function(tree, targeted_set, contamination_set,
  ...
  cols[intersect(names(cols), contamination_set)] <- "blue"
  cols[intersect(names(cols), targeted_set)]      <- "red"
...
run_trend_for_job <- function(otu, clade_info, outdir, tag, method,
  ...
  for (ci in kept){
	...
	in_target_clade_ids <- intersect(in_clade_ids, target_yes)
	grp <- ifelse(rownames(Mat) %in% in_target_clade_ids, group_label, "other")
    ...
    if (lib_lower == "limma"){
      ...
      if (length(feats_for_box) > 0){
        plot_group_box(
          ...
          out_png      = file.path(g_dir, paste0(g_safe, "_limma_box.png"))
        )
      }
      ...
      if (nrow(mat_sig) >= 2 && ncol(mat_sig) >= 1){
        ...
        utils::write.csv(
          data.frame(SampleID = names(cl_lab), Cluster = cl_lab, check.names = FALSE),
    ...
    if (lib_lower %in% c("maaslin2","maaslin","maaslin2 ")){
      ...
      if (length(feats_for_box) > 0){
        plot_group_box(
          ...
          in_group_vec = in_clade_ids,
          group_label  = paste0("Clade_", clade_name),
          out_png      = file.path(g_dir, paste0(g_safe, "_MaAsLin2_box.png"))
        )
      }
...
jobs <- lapply(seq_len(nrow(methods_df)), function(i){
  row <- methods_df[i, , drop = FALSE]
  
  file_tree_str <- as.character(row[["Profile_Tree"]])
  method        <- as.character(row[["Method_Tree"]])
  boot          <- suppressWarnings(as.integer(row[["Boot_Tree"]]))
  tree_raw      <- as.character(row[["BackBacPhylo_Tree"]])
  file_diff_str <- as.character(row[["Profile_Diff"]])
  lib_name      <- as.character(row[["Profile_Library"]])
  
  if (!(method %in% valid_methods)) {
    stop("Invalid method in methods.txt row ", i, ": ", method)
  }
  
  list(
    file_tree_raw = file_tree_str,
    file_tree     = resolve_path(file_tree_str),
    file_diff_raw = file_diff_str,
    file_diff     = resolve_path(file_diff_str),
    method        = method,
    library       = lib_name,
    matrix_label  = matrix_label_from_file(file_diff_str),
    tree_raw      = tree_raw,
    tag           = tools::file_path_sans_ext(basename(file_tree_str)),
...
all_outdirs <- character(0)

for (i in seq_along(jobs)){
  jb <- jobs[[i]]
  
  file_tree_path <- jb$file_tree
  file_tree_raw  <- jb$file_tree_raw
  file_diff_path <- jb$file_diff
  file_diff_raw  <- jb$file_diff_raw
  method         <- jb$method
  lib_name       <- jb$library
  ...
  file_tree_path <- resolve_existing_csv(file_tree_raw)
  if (is.na(file_tree_path)) {
    warning("Skipping job ", i, ": Profile_Tree input file not found: ", file_tree_raw)
    next
  }
  
  file_diff_path <- resolve_existing_csv(file_diff_raw)
  if (is.na(file_diff_path)) {
    warning("Skipping job ", i, ": Profile_Diff input file not found: ", file_diff_raw)
    next
  }
  
  message("\n=== JOB ", i, " ===")
  message("Profile_Tree : ", file_tree_path)
  message("Profile_Diff : ", file_diff_path)
  ...
  if (!is.null(args$`out-root`) && nchar(args$`out-root`) > 0){
    ...
    outdir_root <- dirname(normalizePath(file_tree_path, mustWork = TRUE))
  }
  outdir <- file.path(outdir_root, paste0(tag, "_", method))
  ensure_dir(outdir)
  all_outdirs <- union(all_outdirs, outdir)
  message("Outdir       : ", outdir)
  
  otu_tree <- utils::read.csv(file_tree_path, row.names = 1,
                              check.names = FALSE, stringsAsFactors = FALSE)
  otu_diff <- utils::read.csv(file_diff_path, row.names = 1,
                              check.names = FALSE, stringsAsFactors = FALSE)
  
  common_samples <- intersect(rownames(otu_tree), rownames(otu_diff))
  if (length(common_samples) < 2) {
    warning("Skipping job ", i, ": fewer than 2 shared samples between Profile_Tree and Profile_Diff.")
    next
  }
  if (length(common_samples) < nrow(otu_tree) || length(common_samples) < nrow(otu_diff)) {
    message("  Aligning to ", length(common_samples), " shared samples between Profile_Tree and Profile_Diff.")
  }
  otu_tree <- otu_tree[common_samples, , drop = FALSE]
  otu_diff <- otu_diff[common_samples, , drop = FALSE]
  
  targeted_set <- intersect(common_samples, target_yes)
  ...
  if (need_tree){
    ...
    if (is.na(tree_path_job) || !file.exists(tree_path_job)){
      if (!is.null(global_tree_path) && nchar(global_tree_path)){
        ...
        warning("Skipping job ", i,
                ": phylogenetic method requires a tree, but BackBacPhylo_Tree/--tree did not provide a valid file.")
        next
      }
    }
    ...
    if (methods_to_run[1] %in% c("UniFrac","UniFracW","MPD","MNTD","MPDw","MNTDw")){
      aligned <- align_tree_matrix(otu_tree, ref_tree, label = "Profile_Tree")
      otu_tree <- aligned$mat
  ...
  for (m in methods_to_run){
    ...
    local_tree <- ref_tree
    local_otu_tree <- otu_tree
    
    if (m %in% c("UniFrac","UniFracW","MPD","MNTD","MPDw","MNTDw")){
      aligned <- align_tree_matrix(local_otu_tree, local_tree, label = "Profile_Tree")
      local_otu_tree <- aligned$mat
      local_tree <- aligned$tree
    }
...
finalize_all_merges <- function(out_root) {
  ...
  grp_dirs <- list.dirs(out_root, full.names = TRUE, recursive = TRUE)
...
for (od in all_outdirs) {
  finalize_all_merges(od)
  collect_merged_inputs(od)
}