...
# analysis" field) -- a real gate: even when target.txt's Y-targeted
# samples DO form a valid clade, "Differential analysis = No" skips the
# WHOLE clade-analysis block below (clade detection, the highlighted
# tree, AND run_trend_for_job()) for this block/job -- the pipeline stops
# right after the plain support_tree.nwk/pdf/png. Defaults to TRUE (runs)
# if the field is absent, so blocks that don't set it behave as before.
...
# Derive noise type from Input_Type (always automatic -- there's no
# methods.txt field for this).
input_lc   <- tolower(trimws(input_type))
noise_type <- if (input_lc == "log10") "Gaussian" else if (input_lc == "count") "Poisson" else "Dirichlet"

# --- Differential-analysis library: methods.txt's "Profile_Library" is a
# real gate. If set to "limma" or "MaAsLin2" (case-insensitive), that
# choice is used as-is. Left blank/absent -> falls back to the old
# Input_Type-derived default (log10 -> limma, Count/Frequency ->
# MaAsLin2) so existing methods.txt files without this field keep
# behaving exactly as before. ---
.profile_lib_raw <- cfg[["Profile_Library"]]
if (!is.null(.profile_lib_raw)) .profile_lib_raw <- trimws(sub("#.*$", "", .profile_lib_raw))
.library_source <- "Input_Type"
library_name <- if (!is.null(.profile_lib_raw) && nchar(.profile_lib_raw)) {
  .pl <- tolower(.profile_lib_raw)
  if (.pl == "limma") {
    .library_source <- "Profile_Library"
    "limma"
  } else if (.pl %in% c("maaslin2", "maaslin")) {
    .library_source <- "Profile_Library"
    "MaAsLin2"
  } else {
    stop("methods.txt: Profile_Library must be 'limma' or 'MaAsLin2' (got '",
         .profile_lib_raw, "')\n",
         "  Fix: set Profile_Library = limma  OR  Profile_Library = MaAsLin2\n",
         "  (or leave it blank to auto-derive from Input_Type)")
  }
} else {
  if (input_lc == "log10") "limma" else "MaAsLin2"
}
...
message("Library      : ", library_name, "  [from ", .library_source, "]")
...
# computed above), run_trend_for_job() runs automatically on diff_raw --
# either the same raw/unnormalized Filename data the tree was built from,
# or, when "Input df" names a different file, THAT file's raw data
# instead (see diff_raw below). "Differential analysis = No" stops the
# pipeline after the plain sample tree -- no clade detection, no
# highlighted tree, no TREND. "Profile_Library" controls which library
# runs, see library_name above. ---
...
# --- Differential-analysis input file: methods.txt's "Input df" is a
# real gate. Left blank/absent, or set to the same file as Filename,
# TREND tests the same raw/unnormalized matrix the tree was built from
# (unchanged default behavior). Set to a DIFFERENT file, TREND instead
# tests THAT file's features for the clades Filename's tree found --
# e.g. cluster on BacCount.csv but test which Cell.csv features differ
# within the resulting clades, for the same samples. Either way the tree/
# clade detection always runs on Filename; only the differential-analysis
# feature matrix changes. ---
.input_df_raw <- cfg[["Input df"]]
if (!is.null(.input_df_raw)) .input_df_raw <- trimws(sub("#.*$", "", .input_df_raw))

diff_raw <- raw
if (!is.null(.input_df_raw) && nchar(.input_df_raw) &&
    tolower(.input_df_raw) != tolower(trimws(filename))) {
  diff_csv_path <- file.path(script_dir, "Inputs", .input_df_raw)
  if (!file.exists(diff_csv_path))
    stop("methods.txt: Input df file not found: ", diff_csv_path, "\n",
         "  Fix: set Input df = ", filename,
         "  (or another file in Inputs/), or leave it blank.")
  diff_raw <- as.matrix(read.csv(diff_csv_path, row.names=1,
                                  check.names=FALSE, stringsAsFactors=FALSE))
  diff_raw[] <- as.numeric(diff_raw)
  # Differential analysis has to run on the SAME samples the tree/clades
  # were built from (raw's rownames), whatever feature matrix it draws
  # its columns from.
  missing_from_df <- setdiff(rownames(raw), rownames(diff_raw))
  if (length(missing_from_df))
    stop("methods.txt: Input df '", .input_df_raw, "' is missing ",
         length(missing_from_df), " sample(s) present in Filename='", filename,
         "': ", paste(head(missing_from_df, 5), collapse=", "),
         if (length(missing_from_df) > 5) ", ..." else "")
  diff_raw <- diff_raw[rownames(raw), , drop=FALSE]
  message("Input df     : ", .input_df_raw, "  (", nrow(diff_raw), " x ", ncol(diff_raw),
          ")  [differential analysis tests THIS file; clustering stays on ", filename, "]")
} else {
  message("Input df     : (blank) -- differential analysis tests ", filename, ", same as clustering")
}
...
if (length(target_yes) > 0 && run_diff_analysis) {
  ...
  if (!is.null(diff_raw)) {
    message("\n--- TREND analysis (", library_name, ", ",
            if (identical(diff_raw, raw)) "original/unnormalized input" else "Input df input",
            ") ---")
    run_trend_for_job(
      otu=diff_raw,
...
if (.is_phylo_dist) {
  ...
  if (length(target_yes) > 0 && run_diff_analysis) {
    ...
    if (!is.null(diff_raw)) {
      message("\n--- TREND analysis (Test, ", library_name, ", ",
              if (identical(diff_raw, raw)) "original/unnormalized input" else "Input df input",
              ") ---")
      run_trend_for_job(
        otu=diff_raw,
